...
def main():
    ...
    if (admin_logged_in):
        ...
        match (whether_edit_delete.strip().lower()[0]):
            ...
            case('e'):
                while(True):
                    ...
                    if ((customer_collection.count_documents( { "user" : { "$eq": customer_user.lower() } } )) == 0):
                        log_collection.insert_one( { "log" : "Queried database for existing user during UPDATE!", "when": datetime.now() } )
            ...
            case('d'):
                while(True):
                    ...
                    if ((customer_collection.count_documents( { "user" : { "$eq": customer_user.lower() } } )) == 0):
                        log_collection.insert_one( { "log" : "Queried database for existing user during DELETE!", "when": datetime.now() } )
                        print("The customer username you searched for does not exist!")
                        continue
        ...
        update_or_delete_store = input("\033[1m" + "Would you like to (e)dit or (d)elete items from the store? Or (n)ot? " + "\033[0m")
            ...
            case('e'):
                while(True):
                    ...
                    if ((deku_collection.count_documents( { "item" : { "$eq": store_item_update } } )) == 0):
                        log_collection.insert_one( { "log" : "Queried database for existing STORE ITEM during UPDATE!", "when": datetime.now() } )
            ...
            case('d'):
                while(True):
                    ...
                    if ((deku_collection.count_documents( { "item" : { "$eq": store_item_delete } } )) == 0):
                        log_collection.insert_one( { "log" : "Queried database for existing STORE ITEM during DELETE!", "when": datetime.now() } )
                        print("The item you searched for does not exist!")
                        continue
                ...
                match (whether_collect_buy.strip().lower()[0]):
                        ...
                        while(True):
                            ...
                            match (choice_scavenge.strip().lower()[0]):
                                case('b'):
                                    ...
                                    log_collection.insert_one( { "log" : f"Broke pots to increase wallet by {broken_pots_rupees}!", "when": datetime.now() } )
                                ...
                                case('c'):
                                    ...
                                    log_collection.insert_one( { "log" : f"Cut grass to increase wallet by {cut_grass}!", "when": datetime.now() } )
                                    current_wallet = customer_collection.distinct("wallet", { "user" : customer_username.lower() } )
                                ...
                                case('k'):
                                    ...
                                    log_collection.insert_one( { "log" : f"Cut grass to increase wallet by {kill_enemy}!", "when": datetime.now() } )
                                    current_wallet = customer_collection.distinct("wallet", { "user" : customer_username.lower() } )
                    ...
                    case('b'):
                        while(True):
                            ...
                            # print(entire_store)
                            store_decision = input("Would you like to (b)uy or (s)ell? Or (e)xit? ")
                            match(store_decision.strip().lower()[0]):
                                case('b'):
                                    ...
                                    if ((deku_collection.count_documents( { "item" : { "$eq": item_bought } } )) == 0):
                                        log_collection.insert_one( { "log" : "Queried database for existing STORE ITEM during TRANSACTION!", "when": datetime.now() } )
                                        print("The item you searched for does not exist!")
                                        continue
